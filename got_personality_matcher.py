...
if __name__ == "__main__":
    ...
    closest_character, min_distance = find_closest_character(user_profile, character_profiles)

    print(f"\nYour closest Game of Thrones character is: {closest_character.capitalize()}")

    print(f"You are most like {closest_character.capitalize()}!")
    print(f"Similarity Score: {min_distance:.2f}")
    print("Why? You share these traits:")
    for trait in user_profile:
        print(f"- {trait.capitalize()}: {user_profile[trait]/4} (User) vs. {character_profiles[closest_character][trait]} (Character)")
